...
import { TownEnvironment } from '../environment/TownEnvironment.ts';
import { Town2Environment } from '../environment/Town2Environment';
...
export class SceneManager {
    ...
    public townEnvironment: TownEnvironment | null = null;
    public town2Environment: Town2Environment | null = null;
    ...
    private unloadEnvironments(except: SceneType) {
        ...
        if (except !== 'town' && this.townEnvironment) {
    ...
    public switchScene(sceneName: SceneType, isInit: boolean = false) {
        ...
        if (sceneName === 'dev') {
            ...
            const spawnCellX = 200;
            const spawnCellZ = 117;
            const startX = TOWN2_MIN_X + (spawnCellX + 0.5) * TOWN2_GRID_SIZE;
            const startZ = TOWN2_MIN_Z + (spawnCellZ + 0.5) * TOWN2_GRID_SIZE;
            ...
            this.player.mesh.position.set(startX, 0, startZ);
            this.renderManager.controls.target.set(startX, 1.7, startZ);
            this.renderManager.camera.position.set(startX, 5, startZ + 10);
