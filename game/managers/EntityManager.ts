...
export class EntityManager {
    ...
    constructor(scene: THREE.Scene, environment: Environment, initialConfig: PlayerConfig) {
        // NPC
    ...
    update(delta: number, config: PlayerConfig, playerPosition: THREE.Vector3, environment: Environment) {
        ...
        if (config.showNPC && this.npc) {
            ...
            this.npc.update(delta, eyePos, environment);
        }
        
        if (config.showGuard && this.guard) {
            this.guard.update(delta, playerPosition, environment);
        }

        // Hostiles & Animals
        if (config.showAssassin && this.archer) {
            this.archer.update(delta, environment, [
        ...
        if (this.wolf) this.wolf.update(delta, environment, [{ position: playerPosition.clone() }, { position: this.archer.position.clone() }, { position: this.npc.position.clone() }]);
        if (this.bear) this.bear.update(delta, environment, [{ position: playerPosition.clone() }, { position: this.archer.position.clone() }, { position: this.npc.position.clone() }]);
        if (this.owl) this.owl.update(delta, environment, [{ position: playerPosition.clone() }, { position: this.archer.position.clone() }, { position: this.npc.position.clone() }]);
        if (this.yeti) this.yeti.update(delta, environment, [{ position: playerPosition.clone() }]);
        
        this.deers.forEach(d => d.update(delta, environment, [{ position: playerPosition.clone() }]));
        this.chickens.forEach(c => c.update(delta, environment, [{ position: playerPosition.clone() }]));
        this.pigs.forEach(p => p.update(delta, environment, [{ position: playerPosition.clone() }]));
        this.sheeps.forEach(s => s.update(delta, environment, [{ position: playerPosition.clone() }]));
        this.spiders.forEach(s => s.update(delta, environment, [{ position: playerPosition.clone() }]));
        this.lizards.forEach(l => l.update(delta, environment, [{ position: playerPosition.clone() }]));
        this.horses.forEach(h => h.update(delta, environment, [{ position: playerPosition.clone() }]));

        if (config.showAssassin && this.assassin) {
            this.assassin.update(delta, environment, [{ position: playerPosition.clone() }, { position: this.npc.position.clone() }]);
        }

        // Foundry Battle
        if (this.foundryGuard && this.foundryAssassin) {
            this.foundryGuard.update(delta, playerPosition, environment, [{ position: this.foundryAssassin.position.clone(), isDead: false }]);
            this.foundryAssassin.update(delta, environment, [{ position: this.foundryGuard.position.clone(), isDead: false }]);
        }
    }
