...
export class Pig {
    ...
    update(dt: number, environment: Environment, potentialTargets: { position: THREE.Vector3, isDead?: boolean }[]) {
        ...
        if (this.state === PigState.PATROL && currentSpeed > 0) {
    ...
    private animate(dt: number, currentSpeed: number) {
        ...
        if (currentSpeed > 0) {
            const legSwing = Math.sin(this.walkTime * 2.5) * 0.6;
            ...
            if(parts.body) parts.body.rotation.z = Math.sin(this.walkTime * 2.5) * 0.05;
        } else {
            if(parts.head) parts.head.rotation.x = (this.stateTimer > 4.5 && this.stateTimer < 8.5) ? 0.6 + (Math.sin(this.stateTimer * 5.0) * 0.1) : 0;
    ...
    private die() {
        this.isDead = true; this.state = PigState.DEAD; this.healthBarGroup.visible = false;
        this.hitbox.userData.isSkinnable = true; this.hitbox.userData.material = 'pork';
    ...
    markAsSkinned() {
        ...
        this.model.group.traverse((obj: any) => { if (obj.isMesh && obj.material) { obj.material = obj.material.clone(); obj.material.color.setHex(0x8B4513); } });
    }
}
