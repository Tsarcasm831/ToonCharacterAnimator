...
export class Deer {
    ...
    update(dt: number, environment: Environment, potentialTargets: { position: THREE.Vector3, isDead?: boolean }[]) {
        ...
        if (this.state === DeerState.PATROL) {
            currentSpeed = this.isFleeing ? 5.0 : ((this.stateTimer % 5.0 > 3.0) ? 0 : this.moveSpeedVal);
        ...
        if (currentSpeed > 0) {
            ...
            if (toTarget.length() > 0.1) {
                ...
                while (diff > Math.PI) diff -= Math.PI * 2;
                this.rotationY += diff * (this.isFleeing ? 8.0 : 3.0) * dt; 
    ...
    private animate(dt: number, currentSpeed: number) {
        ...
        if (currentSpeed > 0) {
            ...
            if(parts.tail) parts.tail.rotation.x = Math.PI/4 + Math.sin(time * 5) * 0.2;
        } else {
            const scanPhase = (this.stateTimer % 5.0) > 3.0;
            if (parts.head) {
                if (scanPhase) { parts.head.rotation.x = -0.3; parts.head.rotation.y = Math.sin(this.stateTimer * 2.0) * 0.3; }
                else { parts.head.rotation.x = 0.8; parts.head.rotation.y = 0; }
            }
    ...
    takeDamage(amount: number) {
        if (this.isDead) return;
        this.health -= amount; this.isFleeing = true; this.stateTimer = 0; this.findPatrolPoint();
    ...
    private die() {
        this.isDead = true; this.state = DeerState.DEAD; this.healthBarGroup.visible = false;
        this.hitbox.userData.isSkinnable = true; this.hitbox.userData.material = 'venison';
        this.hitbox.children.forEach(child => { child.userData.isSkinnable = true; child.userData.material = 'venison'; });
        this.model.group.rotation.z = Math.PI / 2; this.model.group.position.y = 0.5; this.hitbox.position.y = -0.6;
    }

    markAsSkinned() {
        this.isSkinned = true; this.hitbox.userData.isSkinnable = false;
        this.model.group.traverse((obj: any) => { if (obj.isMesh && obj.material) { obj.material = obj.material.clone(); obj.material.color.setHex(0x5A2D0C); } });
    }
}
