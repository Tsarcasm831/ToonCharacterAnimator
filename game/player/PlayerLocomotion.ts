...
export class PlayerLocomotion {
    ...
    update(dt: number, input: PlayerInput, cameraAngle: number, obstacles: THREE.Object3D[], isInCombat: boolean = false) {
        ...
        if (!PlayerUtils.isWithinBounds(pos)) {
            this.position.copy(PlayerUtils.constrainToBounds(pos));
            this.previousPosition.copy(this.position);
        }
        ...
        const waterDepth = PlayerUtils.getWaterDepth(pos.x, pos.z);
        ...
        if (this.isJumping) {
            ...
            if (this.position.y > groundHeight) {
                ...
                const jumpPowerMod = isInWater ? Math.max(0.4, 1.0 - (waterDepth * 0.5)) : 1.0;
