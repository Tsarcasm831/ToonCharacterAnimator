...
export class Lizard {
    ...
    private readonly collisionSize = new THREE.Vector3(0.5, 0.4, 1.2);
    ...
    constructor(scene: THREE.Scene, initialPos: THREE.Vector3) {
        ...
        if (!ObjectFactory.createLizardModel) lizardData.group.scale.set(0.4, 0.4, 0.4);
    ...
    update(dt: number, environment: Environment, potentialTargets: { position: THREE.Vector3, isDead?: boolean }[]) {
        ...
        if (this.state === LizardState.PATROL && currentSpeed > 0) {
    ...
    private die() {
        this.isDead = true; this.state = LizardState.DEAD; this.healthBarGroup.visible = false;
        this.hitbox.userData.isSkinnable = true; this.hitbox.userData.material = 'scales';
        this.model.group.rotation.z = Math.PI; this.model.group.position.y = 0.2; this.hitbox.position.y = -0.5;
    }

    markAsSkinned() {
        this.isSkinned = true; this.hitbox.userData.isSkinnable = false;
        this.model.group.traverse((obj: any) => { if (obj.isMesh && obj.material) { obj.material = obj.material.clone(); obj.material.color.setHex(0x333333); } });
    }
}
