...
import { BUILDING_COSTS, BuildingCost } from '../../data/constants';
...
export class BuilderManager {
    ...
    private isPlacementLocked: boolean = false;
    private _rayCoords = new THREE.Vector2();
    
    public onBuild?: (type: string, pos: THREE.Vector3, rot: number) => void;
    public onCheckCost?: (cost: BuildingCost) => boolean;
    public onDeductCost?: (cost: BuildingCost) => void;
    ...
    selectPlacementLocation(): boolean {
        if (!this.isActive || !this.ghostMesh) return false;
        if (!this.isPlacementLocked) {
            this.isPlacementLocked = true;
            this.updateGhostLockState(true);
            return false;
        }
        return true;
    }

    private updateGhostLockState(isLocked: boolean) {
        if (!this.ghostMesh) return;

        this.ghostMesh.userData = {
            ...this.ghostMesh.userData,
            isPlacementLocked: isLocked
        };

        this.ghostMesh.traverse(obj => {
            const mesh = obj as THREE.Mesh;
            if (!(mesh as any).material) return;

            const material = mesh.material as THREE.Material | THREE.Material[];
            const materials = Array.isArray(material) ? material : [material];
            materials.forEach(mat => {
                const anyMat = mat as THREE.MeshStandardMaterial & { transparent?: boolean; opacity?: number; emissive?: THREE.Color; emissiveIntensity?: number };
                anyMat.transparent = true;
                anyMat.opacity = isLocked ? 0.85 : 0.55;
                if (anyMat.emissive) {
                    anyMat.emissive = new THREE.Color(isLocked ? 0x3cff8f : 0x88ffcc);
                    anyMat.emissiveIntensity = isLocked ? 0.65 : 0.35;
                }
            });
        });
    }
    ...
    private updateGhost() {
        ...
        this.ghostMesh.rotation.y = this.ghostRotation;
        this.updateGhostLockState(this.isPlacementLocked);
    ...
    build(environment: { obstacles: THREE.Object3D[] }) {
        ...
        // Check building cost
        const cost = BUILDING_COSTS[this.currentType];
        if (cost && this.onCheckCost) {
            if (!this.onCheckCost(cost)) {
                console.log('BuilderManager.build: Insufficient resources for', this.currentType, cost);
                return;
            }
        }
        ...
        // Deduct cost after successful build
        if (cost && this.onDeductCost) {
            this.onDeductCost(cost);
        }
