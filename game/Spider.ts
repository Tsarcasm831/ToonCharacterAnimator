...
export class Spider {
    ...
    update(dt: number, environment: Environment, potentialTargets: { position: THREE.Vector3, isDead?: boolean }[]) {
        ...
        if (this.state === SpiderState.PATROL && currentSpeed > 0) {
    ...
    private animate(dt: number, moveSpeed: number) {
        ...
        if (moveSpeed > 0) {
            if (parts.legL1) {
                const groupA = Math.sin(time), groupB = Math.sin(time + Math.PI), lift = 0.5;
                parts.legL1.rotation.x = groupA * lift; parts.legR2.rotation.x = groupA * lift; parts.legL3.rotation.x = groupA * lift; parts.legR4.rotation.x = groupA * lift;
                parts.legR1.rotation.x = groupB * lift; parts.legL2.rotation.x = groupB * lift; parts.legR3.rotation.x = groupB * lift; parts.legL4.rotation.x = groupB * lift;
            }
    ...
    private die() {
        this.isDead = true; this.state = SpiderState.DEAD; this.healthBarGroup.visible = false;
        this.hitbox.userData.isSkinnable = true; this.hitbox.userData.material = 'silk';
        this.model.group.rotation.x = Math.PI; this.model.group.position.y = 0.5; this.hitbox.position.y = -0.5;
    }

    markAsSkinned() {
        this.isSkinned = true; this.hitbox.userData.isSkinnable = false;
        this.model.group.traverse((obj: any) => { if (obj.isMesh && obj.material) { obj.material = obj.material.clone(); obj.material.color.setHex(0x555555); } });
    }
}
