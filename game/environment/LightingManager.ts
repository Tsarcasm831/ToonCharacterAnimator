...
export class LightingManager {
    ...
    private readonly skyMaxRadius = 190;
    ...
    public setSkySphereRadius(radius: number) {
        const safeRadius = THREE.MathUtils.clamp(radius, this.skyBaseRadius, this.skyMaxRadius);
    ...
    private updateDayNight(dt: number, config: PlayerConfig, focusPosition?: THREE.Vector3) {
        ...
        const anchorY = focusPosition?.y ?? 0;
        const anchorZ = focusPosition?.z ?? 0;
        this.skySphere.position.set(anchorX, anchorY, anchorZ);
        ...
        const shadowSunY = Math.max(sunY, 35);
        this.sunLight.position.set(anchorX + sunX, anchorY + shadowSunY, anchorZ + sunZ);
        this.sunLight.target.updateMatrixWorld();

        const lightIntensity = Math.max(0.35, sunAltitude * 0.8 + 0.1);
