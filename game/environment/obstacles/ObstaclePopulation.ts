...
import { TreeData, RockData, ENV_CONSTANTS, BIOME_DATA } from '../EnvironmentTypes';
...
const isGrassGround = (x: number, z: number): boolean => {
...
export async function initWorldScatter(ctx: ObstacleInitContext, options?: PopulateOptions) {
    ...
    for (const tree of ctx.trees.values()) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            if (Math.random() > 0.4 && isGrassGround(sx, sz)) {
    ...
    for (const rock of ctx.rocks.values()) {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            if (Math.random() > 0.8 && isGrassGround(sx, sz)) {
    ...
    for (let i = 0; i < 160; i++) {
        ...
        // Use the same grass ground check that includes pond checking
