...
uniform float uPresence; // 0.0 to 1.0 (fade in/out based on player biome)
...
void main() {
    ...
    // BIOME BOUNDS (Local to the system at 0, 0, -40)
    float boxWidth = 40.0;
    float boxHeight = 25.0; // Taller box for better vertical coverage
    
    // 1. GENTLE FALLING LOGIC
    // Base speed is very slow. Intensity adds a bit more speed.
    // uTime * speed * fallSpeed ensures positive downward distance.
    float fallSpeed = mix(0.8, 3.5, uIntensity);
    float fallDistance = uTime * speed * fallSpeed;
    
    // pos.y starts at initial random height, subtract fallDistance, then mod to wrap
    pos.y = mod(pos.y - fallDistance, boxHeight);
    
    // 2. REFINED WIND & DRIFT
    // Horizontal drift is subtle unless gale is high.
    // Frequency reduced to avoid "violent" shaking.
    float driftX = sin(uTime * 0.2 + offset.x) * 1.0 * uIntensity;
    float driftZ = cos(uTime * 0.15 + offset.z) * 1.0 * uIntensity;
    
    // Gale force adds directional push rather than vibration
...
void main() {
    ...
    // Harder alpha falloff for crisper flakes (less blurry)
...
export class SnowSystem {
    ...
    constructor(parent: THREE.Object3D) {
        ...
        for (let i = 0; i < this.count; i++) {
            ...
            speeds[i] = 0.7 + Math.random() * 0.6; // Uniform scale, no negative speeds
    ...
    update(dt: number, playerPosition: THREE.Vector3) {
        // 1. Weather State Machine (Cycle less often: every 60-120 seconds)
        ...
        // 2. Biome Presence Logic
        // Frostfell Peaks (Grid 0, -1) bounds: X[-20, 20], Z[-60, -20]
        ...
        // Fade presence in and out gradually
        this.presence = THREE.MathUtils.lerp(this.presence, presenceTarget, dt * 0.8);

        // 3. Smooth Lerp Intensity (Transitions are now much slower)
        ...
        // Hide completely if opacity is negligible
