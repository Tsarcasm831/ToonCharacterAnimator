...
export class FireballAction {
    static animate(player: any, parts: any, dt: number, damp: number) {
        ...
        if (t < 0.4) {
            ...
            // Swirl hands around a center point in front of chest
            ...
            // Base stance: Stable, slight crouch, no funky leaning
            ...
            // Arms: Hands mostly in front, elbows out slightly, swirling
            // Right Arm
            ...
            // Hands: Cupped facing each other
            ...
            // Jitter for energy build-up (Direct set to avoid drift)
            ...
            // Push body forward into the cast
            ...
            // Arms extend fully forward
            // Right
            parts.rightArm.rotation.x = lerp(parts.rightArm.rotation.x, -1.57, thrustDamp); // Shoulder height
            parts.rightArm.rotation.y = lerp(parts.rightArm.rotation.y, -0.3, thrustDamp); // Converge center
            parts.rightArm.rotation.z = lerp(parts.rightArm.rotation.z, -0.1, thrustDamp); 
            parts.rightForeArm.rotation.x = lerp(parts.rightForeArm.rotation.x, 0.0, thrustDamp); // Locked elbow
            ...
            // Palms open forward (Blast pose)
            // Rotate X -90 to point palm (Z+) forward when arm is forward
            ...
            // Reset Y to neutral relative to forearm
