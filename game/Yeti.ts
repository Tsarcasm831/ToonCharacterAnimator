...
export class Yeti {
    ...
    private readonly collisionSize = new THREE.Vector3(2.5, 3.0, 1.4);
    ...
    constructor(scene: THREE.Scene, initialPos: THREE.Vector3) {
        ...
        headHitbox.position.set(0, 3.0, 0.3);
    ...
    private animate(dt: number, moveSpeed: number) {
        ...
        if (parts.body) { parts.body.rotation.x = -Math.PI / 2.5; parts.body.position.y = 1.5; }
    ...
    private die() {
        this.isDead = true; this.state = YetiState.DEAD; this.healthBarGroup.visible = false;
        this.hitbox.userData.isSkinnable = true; this.hitbox.userData.material = 'thick_white_fur';
        this.hitbox.children.forEach(child => { child.userData.isSkinnable = true; child.userData.material = 'thick_white_fur'; });
        this.model.group.rotation.x = Math.PI / 2; this.model.group.position.y = 0.5; this.hitbox.position.y = -10; 
    }

    markAsSkinned() {
        this.isSkinned = true; this.hitbox.userData.isSkinnable = false;
        this.model.group.traverse((obj: any) => { if (obj.isMesh && obj.material) { obj.material = obj.material.clone(); obj.material.color.setHex(0xaaaaaa); } });
    }
}
