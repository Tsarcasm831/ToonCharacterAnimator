...
import { DoorManager } from '../environment/DoorManager';
import { Door } from '../environment/objects/Door';
import { GameLoopManager } from '../gameLoop';
...
export interface NpcInteractionResponse {
    ...
    mode?: 'dialogue' | 'trade' | 'shop' | 'forge';
...
export class Game {
    ...
    public onDialogueTrigger?: (content: string | DialogueContent) => void;
    ...
    constructor(container: HTMLElement, initialConfig: PlayerConfig, initialManualInput: Partial<PlayerInput>, initialInventory: (InventoryItem | null)[], activeScene: SceneType) {
        ...
        } else if (activeScene === 'starter' || activeScene === 'singleBiome') {
            this.setupSingleBiomeLogPickup();
        } else if (activeScene === 'town') {
            this.setupTownScene();
        } else if (activeScene === 'town2' || activeScene === 'gameLoop') {
        ...
        (window as any).debugSpawnDoor = () => {
            console.log('[Game] Debug: Spawning door at player position');
            if (this.doorManager && this.player) {
                const playerPos = this.player.mesh.position.clone();
                const door = new Door(playerPos, this.player.mesh.rotation.y);
                this.doorManager.addDoor(door);
                this.renderManager.scene.add(door.mesh);
                console.log('[Game] Debug: Door spawned at player position');
            }
        };
    }
    ...
    private handleTownEntry(input: PlayerInput): boolean {
        if (this.sceneManager.activeScene === 'land') {
            if (!this.onEnterTown2 || !this.sceneManager.landEnvironment) {
                this.enterTownTriggered = false;
                return false;
            }

            const landEntry = this.sceneManager.landEnvironment.getTown2EntryZone?.();
            if (!landEntry) {
                this.enterTownTriggered = false;
                return false;
            }

            const dx = this.player.mesh.position.x - landEntry.position.x;
            const dz = this.player.mesh.position.z - landEntry.position.z;
            const inRange = (dx * dx + dz * dz) <= (landEntry.radius * landEntry.radius);
            ...
            this.onInteractionUpdate?.('Press E to Enter Town2', null);
    ...
    public startCombatEncounterRound(enemyType: string, enemyCount: number): boolean {
        ...
        const friendlyEntities = this.entityManager.getEntitiesForScene('combat');
        const reservedCells: { r: number; c: number }[] = [];

        for (const entity of friendlyEntities) {
            const pos = entity?.position || entity?.mesh?.position || entity?.model?.group?.position;
            if (!pos) continue;
            const gridPos = arena.getGridPosition(pos);
            if (gridPos) {
                reservedCells.push(gridPos);
            }
        }
        ...
        const disposeEntity = (entity: any) => {
            ...
            if (entity?.model?.group?.parent) {
                entity.model.group.parent.remove(entity.model.group);
            }
        };

        this.entityManager.imps.forEach(disposeEntity);
        this.entityManager.imps = [];

        this.config.isAssassinHostile = true;
        arena.setCombatStarted(true);
        arena.clearOccupiedCells();

        this.combatSystem.reset();
        this.combatManager.clearSelection();
        this.entityManager.spawnCombatEncounter(enemyType, enemyCount, arena, reservedCells);

        const enemies = [...this.entityManager.imps];
        if (friendlyEntities.length === 0 || enemies.length === 0) {
            this.combatInitialized = false;
            return false;
        }

        this.combatSystem.initializeCombat(null, friendlyEntities, enemies, arena);
    ...
    public switchScene(sceneName: SceneType, isInit: boolean = false) {
        ...
        if (sceneName === 'town2' || sceneName === 'gameLoop') {
            this.setupTown2Scene();
        }
        if (sceneName === 'starter' || sceneName === 'singleBiome') {
            this.setupSingleBiomeLogPickup();
        }
    ...
    private setupTown2Scene() {
        const town2Env = this.sceneManager.town2Environment;
        if (!town2Env) return;
        this.entityManager.initTown2Entities(town2Env, this.config);
    }

    private setupSingleBiomeLogPickup() {
        const singleBiomeEnv = this.sceneManager.singleBiomeEnvironment;
        if (!singleBiomeEnv) return;
        singleBiomeEnv.onLogPickedUp = () => {
    ...
    private animate(time: number = 0) {
        ...
        let cameraCollisionObstacles: THREE.Object3D[] = [];
        ...
        if (currentEnv) {
            ...
            cameraCollisionObstacles = combinedObstacles;
        ...
        this.cameraManager.updatePosition(this.sceneManager.activeScene, cameraCollisionObstacles);
        ...
        if (this.player.isTalking) this.onInteractionUpdate?.(null, null);
            ...
            const customLabel = target?.group?.userData?.interactionLabel;
            
            const label = customLabel ?? ((targetName === 'Shopkeeper' || target instanceof Shopkeeper)
                ? 'Press E to Chat'
            ...
            if (input.interact) { 
                const customResponse = this.onNpcInteraction?.({
                    scene: this.sceneManager.activeScene,
                    target
                });
                const interactionMode = customResponse?.mode ?? target?.group?.userData?.interactionMode;
                const dialogue = customResponse?.dialogue ?? target?.group?.userData?.dialogue;
                const suppressDefault = !!customResponse?.suppressDefault;

                if ((targetName === 'LowLevelCityGuard' || target instanceof LowLevelCityGuard) && !suppressDefault) { 
                ...
                if (interactionMode === 'trade') {
                    ...
                    this.onTradeTrigger?.();
                } else if (interactionMode === 'shop') {
                    this.player.isTalking = false;
                    this.onShopkeeperTrigger?.();
                } else if (interactionMode === 'forge') {
                    this.player.isTalking = false;
                    this.onForgeTrigger?.();
                } else if (dialogue) {
                    this.player.isTalking = true;
                    this.onDialogueTrigger?.(dialogue);
                } else if (!suppressDefault && (targetName === 'LowLevelCityGuard' || target instanceof LowLevelCityGuard)) {
                    this.player.isTalking = true;
                ...
                else if (!suppressDefault && (targetName === 'Blacksmith' || target instanceof Blacksmith)) {
                    this.player.isTalking = false;
                    this.onTradeTrigger?.();
                }
                else if (!suppressDefault && (targetName === 'Shopkeeper' || target instanceof Shopkeeper)) {
                    this.player.isTalking = false;
                    this.onShopkeeperTrigger?.();
                }
                else {
        ...
        else if (this.player.pickupTarget) {
            ...
            if (target) {
                let label = 'Interact';
                if (target.userData.interactType === 'forge') label = 'Press E to Forge';
                else if (target.userData.interactType === 'door') {
                    const door = target.userData.door;
                    if (door && typeof door.getInteractionPrompt === 'function') {
                        label = door.getInteractionPrompt();
                    } else {
                        label = 'Press E to Use Door';
                    }
                }
                this.onInteractionUpdate?.(label, null);
        ...
        this.renderManager.render();
        this.gameLoopManager.update(delta);
        this.onUpdate?.(delta);
    }
}
