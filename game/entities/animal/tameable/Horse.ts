...
export class Horse {
    scene: THREE.Scene; group: THREE.Group; model: any; position: THREE.Vector3 = new THREE.Vector3(); rotationY: number = 0; state: HorseState = HorseState.PATROL; stateTimer: number = 0; targetPos: THREE.Vector3 = new THREE.Vector3(); isDead: boolean = false; isSkinned: boolean = false; maxHealth: number = 40; health: number = 40; hitbox: THREE.Group; 
    private healthBarGroup: THREE.Group; 
    private uiRefs: any;
    private walkTime: number = 0; private moveSpeedVal: number = 2.8; private readonly collisionSize = new THREE.Vector3(1.0, 1.2, 2.4); private stuckTimer: number = 0; private lastStuckPos: THREE.Vector3 = new THREE.Vector3();
    ...
    constructor(scene: THREE.Scene, initialPos: THREE.Vector3) {
        ...
        // Accurate Horse Hull
        const bodyBox = new THREE.Mesh(new THREE.BoxGeometry(0.85, 1.0, 1.8), hitboxMat); 
        bodyBox.position.y = 1.0; 
        bodyBox.userData = { type: 'creature' }; 
        this.hitbox.add(bodyBox);
        
        const neckBox = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.9, 0.45), hitboxMat); 
        neckBox.position.set(0, 1.6, 0.95); 
        neckBox.rotation.x = -Math.PI / 6.5; 
        neckBox.userData = { type: 'creature' }; 
        this.hitbox.add(neckBox);
        
        const headBox = new THREE.Mesh(new THREE.BoxGeometry(0.42, 0.45, 0.65), hitboxMat); 
        headBox.position.set(0, 2.2, 1.2); 
        headBox.userData = { type: 'creature' }; 
        this.hitbox.add(headBox);

        // Enhanced Health Bar
        this.healthBarGroup = new THREE.Group(); 
        this.healthBarGroup.position.set(0, 3.0, 0); 
        this.uiRefs = PlayerUtils.createHealthBar(this.healthBarGroup, this.maxHealth, 0x8B4513, 'Horse');
