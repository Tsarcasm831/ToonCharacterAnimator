...
export class Sheep {
    scene: THREE.Scene; group: THREE.Group; model: any; position: THREE.Vector3 = new THREE.Vector3(); rotationY: number = 0; state: SheepState = SheepState.PATROL; stateTimer: number = 0; targetPos: THREE.Vector3 = new THREE.Vector3(); isDead: boolean = false; isSkinned: boolean = false; maxHealth: number = 25; health: number = 25; hitbox: THREE.Group; 
    private healthBarGroup: THREE.Group; 
    private uiRefs: any;
    private walkTime: number = 0; private moveSpeedVal: number = 1.0; private readonly collisionSize = new THREE.Vector3(1.1, 1.1, 1.6); private stuckTimer: number = 0; private lastStuckPos: THREE.Vector3 = new THREE.Vector3();
    ...
    constructor(scene: THREE.Scene, initialPos: THREE.Vector3) {
        ...
        // Bulky Wool Hull
        const bodyBox = new THREE.Mesh(new THREE.BoxGeometry(0.85, 0.9, 1.2), hitboxMat); 
        bodyBox.position.y = 0.75; 
        bodyBox.userData = { type: 'creature' }; 
        this.hitbox.add(bodyBox);
        
        const headBox = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.45, 0.5), hitboxMat); 
        headBox.position.set(0, 1.05, 0.65); 
        ...
        this.uiRefs = PlayerUtils.createHealthBar(this.healthBarGroup, this.maxHealth, 0xFFFDD0, 'Sheep');
    ...
    private die() { this.isDead = true; this.state = SheepState.DEAD; this.healthBarGroup.visible = false; this.hitbox.userData.isSkinnable = true; this.hitbox.userData.material = 'wool'; this.hitbox.children.forEach(child => { child.userData.isSkinnable = true; child.userData.material = 'wool'; }); this.model.group.rotation.z = Math.PI / 2; this.model.group.position.y = 0.4; this.hitbox.position.y = -0.5; }
