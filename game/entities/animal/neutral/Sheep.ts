...
export enum SheepState { IDLE, PATROL, FLEE, DEAD }

export class Sheep {
    scene: THREE.Scene; group: THREE.Group; model: any; position: THREE.Vector3 = new THREE.Vector3(); rotationY: number = 0; state: SheepState = SheepState.PATROL; stateTimer: number = 0; targetPos: THREE.Vector3 = new THREE.Vector3(); isDead: boolean = false; isSkinned: boolean = false; maxHealth: number = 25; health: number = 25; hitbox: THREE.Group; private healthBarGroup: THREE.Group; private healthBarFill: THREE.Mesh; private walkTime: number = 0; private moveSpeedVal: number = 1.0; private readonly collisionSize = new THREE.Vector3(1.1, 1.1, 1.6); private stuckTimer: number = 0; private lastStuckPos: THREE.Vector3 = new THREE.Vector3();
    ...
    constructor(scene: THREE.Scene, initialPos: THREE.Vector3) {
        ...
        const sheepData = ObjectFactory.createSheepModel ? ObjectFactory.createSheepModel(0xFFFDD0) : ObjectFactory.createBearModel(0xFFFDD0); this.group = new THREE.Group(); this.group.add(sheepData.group); this.model = sheepData;
        ...
        const bodyHitbox = new THREE.Mesh(new THREE.BoxGeometry(1.1, 1.1, 1.6), hitboxMat); bodyHitbox.position.y = 0.8; bodyHitbox.userData = { type: 'creature' }; this.hitbox.add(bodyHitbox);
        const headHitbox = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.6, 0.7), hitboxMat); headHitbox.position.set(0, 1.1, 1.0); headHitbox.userData = { type: 'creature' }; this.hitbox.add(headHitbox);
    ...
    private animate(dt: number, currentSpeed: number) { const parts = this.model.parts; if (currentSpeed > 0) { const legSwing = Math.sin(this.walkTime * 1.5) * 0.5; if(parts.legFR) parts.legFR.rotation.x = legSwing; if(parts.legBL) parts.legBL.rotation.x = legSwing; if(parts.legFL) parts.legFL.rotation.x = -legSwing; if(parts.legBR) parts.legBR.rotation.x = -legSwing; if(parts.head) parts.head.rotation.x = 0; if(parts.body) parts.body.position.y = 0.8 + Math.abs(Math.cos(this.walkTime * 1.5)) * 0.05; } else { if(parts.head) parts.head.rotation.x = (Math.sin(this.stateTimer * 0.5) > 0.5) ? 0.5 : 0; const breath = Math.sin(this.stateTimer * 2.0) * 0.02; if(parts.body) parts.body.scale.set(1 + breath, 1 + breath, 1 + breath); } }
    ...
    private die() { this.isDead = true; this.state = SheepState.DEAD; this.healthBarGroup.visible = false; this.hitbox.userData.isSkinnable = true; this.hitbox.userData.material = 'wool'; this.model.group.rotation.z = Math.PI / 2; this.model.group.position.y = 0.4; this.hitbox.position.y = -0.5; }
    markAsSkinned() { this.isSkinned = true; this.hitbox.userData.isSkinnable = false; this.model.group.traverse((obj: any) => { if (obj.isMesh && obj.material) { obj.material = obj.material.clone(); obj.material.color.setHex(0xffc0cb); } }); }
}
