...
export class Pig {
    scene: THREE.Scene; group: THREE.Group; model: any; position: THREE.Vector3 = new THREE.Vector3(); rotationY: number = 0; state: PigState = PigState.PATROL; stateTimer: number = 0; targetPos: THREE.Vector3 = new THREE.Vector3(); isDead: boolean = false; isSkinned: boolean = false; maxHealth: number = 30; health: number = 30; hitbox: THREE.Group; private healthBarGroup: THREE.Group; private healthBarFill: THREE.Mesh; private walkTime: number = 0; private moveSpeedVal: number = 1.8; private readonly collisionSize = new THREE.Vector3(1.0, 0.9, 1.5); private stuckTimer: number = 0; private lastStuckPos: THREE.Vector3 = new THREE.Vector3();
    ...
    constructor(scene: THREE.Scene, initialPos: THREE.Vector3) {
        ...
        const bodyHitbox = new THREE.Mesh(new THREE.BoxGeometry(1.0, 0.9, 1.5), hitboxMat); bodyHitbox.position.y = 0.6; bodyHitbox.userData = { type: 'creature' }; this.hitbox.add(bodyHitbox);
        const headHitbox = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.7, 0.7), hitboxMat); headHitbox.position.set(0, 0.8, 1.0); headHitbox.userData = { type: 'creature' }; this.hitbox.add(headHitbox);
        const snoutHitbox = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.3, 0.3), hitboxMat); snoutHitbox.position.set(0, 0.7, 1.4); snoutHitbox.userData = { type: 'creature' }; this.hitbox.add(snoutHitbox);
        this.healthBarGroup = new THREE.Group(); this.healthBarGroup.position.set(0, 1.5, 0); const bg = new THREE.Mesh(new THREE.PlaneGeometry(1.0, 0.15), new THREE.MeshBasicMaterial({ color: 0x330000, side: THREE.DoubleSide })); this.healthBarGroup.add(bg); const fgGeo = new THREE.PlaneGeometry(0.96, 0.11); fgGeo.translate(0.48, 0, 0); this.healthBarFill = new THREE.Mesh(fgGeo, new THREE.MeshBasicMaterial({ color: 0x33ff33, side: THREE.DoubleSide })); this.healthBarFill.position.set(-0.48, 0, 0.01); this.healthBarGroup.add(this.healthBarFill); this.group.add(this.healthBarGroup);
    ...
    private die() { this.isDead = true; this.state = PigState.DEAD; this.healthBarGroup.visible = false; this.hitbox.userData.isSkinnable = true; this.hitbox.userData.material = 'pork'; this.model.group.rotation.z = Math.PI / 2; this.model.group.position.y = 0.4; this.hitbox.position.y = -0.5; }
