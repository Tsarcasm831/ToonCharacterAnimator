...
export class Deer {
    ...
    constructor(scene: THREE.Scene, initialPos: THREE.Vector3) {
        ...
        const deerData = ObjectFactory.createDeerModel ? ObjectFactory.createDeerModel(0xC19A6B) : ObjectFactory.createBearModel(0xC19A6B); this.group = new THREE.Group(); this.group.add(deerData.group); this.model = deerData; this.hitbox = new THREE.Group(); this.hitbox.userData = { type: 'creature', parent: this }; this.group.add(this.hitbox);
        const hitboxMat = new THREE.MeshBasicMaterial({ visible: false, wireframe: true, color: 0xff0000 });
        const bodyHitbox = new THREE.Mesh(new THREE.BoxGeometry(0.8, 1.0, 1.8), hitboxMat); bodyHitbox.position.y = 1.2; bodyHitbox.userData = { type: 'creature' }; this.hitbox.add(bodyHitbox);
        const neckHitbox = new THREE.Mesh(new THREE.BoxGeometry(0.4, 1.2, 0.5), hitboxMat); neckHitbox.position.set(0, 1.9, 1.2); neckHitbox.rotation.x = -Math.PI / 6; neckHitbox.userData = { type: 'creature' }; this.hitbox.add(neckHitbox);
        const headHitbox = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.6, 0.8), hitboxMat); headHitbox.position.set(0, 2.5, 1.5); headHitbox.userData = { type: 'creature' }; this.hitbox.add(headHitbox);
    ...
    markAsSkinned() { this.isSkinned = true; this.hitbox.userData.isSkinnable = false; this.model.group.traverse((obj: any) => { if (obj.isMesh && obj.material) { obj.material = obj.material.clone(); obj.material.color.setHex(0x5A2D0C); } }); }
}
