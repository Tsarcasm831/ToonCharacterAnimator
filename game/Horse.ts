...
export class Horse {
    ...
    private readonly collisionSize = new THREE.Vector3(1.0, 1.2, 2.4);
    ...
    constructor(scene: THREE.Scene, initialPos: THREE.Vector3) {
        ...
        neckHitbox.rotation.x = -Math.PI / 8;
    ...
    private animate(dt: number, currentSpeed: number) {
        ...
        if (currentSpeed > 0) {
            const legSwing = Math.sin(this.walkTime * 1.2 * 2.0) * 0.5;
            ...
            if(parts.body) parts.body.position.y = 1.1 + Math.abs(Math.cos(this.walkTime * 1.2 * 2.0)) * 0.1;
            if(parts.tail) parts.tail.rotation.z = Math.sin(this.walkTime * 1.2 * 2.0) * 0.2;
        } else {
    ...
    private die() {
        this.isDead = true; this.state = HorseState.DEAD; this.healthBarGroup.visible = false;
        this.hitbox.userData.isSkinnable = true; this.hitbox.userData.material = 'leather';
