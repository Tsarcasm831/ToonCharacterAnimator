...
export class Wolf {
    ...
    constructor(scene: THREE.Scene, initialPos: THREE.Vector3) {
        ...
        if (!this.isInsideGrove(spawnPos)) spawnPos.copy(this.GROVE_CENTER);
    ...
    update(dt: number, environment: Environment, potentialTargets: { position: THREE.Vector3, isDead?: boolean }[]) {
        ...
        for (const t of potentialTargets) {
            if (t.isDead || !this.isInsideGrove(t.position)) continue;
        ...
        if (this.state === WolfState.PATROL) {
            moveSpeed = 2.0;
            if (this.position.distanceTo(this.targetPos) < 1.0 || this.stateTimer > 10.0 || !this.isInsideGrove(this.position)) {
                this.findPatrolPoint();
                this.stateTimer = 0;
            }
    ...
    private findPatrolPoint() {
        if (!this.isInsideGrove(this.position)) { this.targetPos.copy(this.GROVE_CENTER); return; }
        const range = 15;
        const candidate = new THREE.Vector3(this.position.x + (Math.random() - 0.5) * range, 0, this.position.z + (Math.random() - 0.5) * range);
    ...
    private animate(dt: number, moveSpeed: number) {
        ...
        if (moveSpeed > 0) {
            ...
            parts.body.position.y = 0.6 + Math.abs(Math.cos(this.walkTime * 2.0)) * 0.1;
    ...
    takeDamage(amount: number) {
        ...
        else { setTimeout(() => { if (!this.isDead) { this.model.parts.body.material.emissiveIntensity = 0; } }, 100); this.state = WolfState.CHASE; }
    }

    private die() {
        this.isDead = true; this.state = WolfState.DEAD; this.healthBarGroup.visible = false;
        this.hitbox.userData.isSkinnable = true; this.hitbox.userData.material = 'flesh';
        this.hitbox.children.forEach(child => { child.userData.isSkinnable = true; child.userData.material = 'flesh'; });
        this.model.group.rotation.z = Math.PI / 2; this.model.group.position.y = 0.15; this.hitbox.position.y = -0.4;
    }
