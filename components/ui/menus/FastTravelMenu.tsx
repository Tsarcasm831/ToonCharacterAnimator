...
export const FastTravelMenu: React.FC<FastTravelMenuProps> = ({ activeScene, onTravel, onClose }) => {
  return (
    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[100] w-64 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl overflow-hidden p-2 animate-in zoom-in-95 duration-200">
      ...
      <button 
        onClick={() => onTravel('singleBiome')} 
        className={`w-full px-4 py-4 rounded-2xl text-left text-xs font-black uppercase tracking-widest transition-all flex items-center justify-between group mt-1 ${activeScene === 'singleBiome' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-300 hover:bg-white/5'}`}
      >
        <span>Single Biome</span>
      ...
      <button 
        onClick={() => onTravel('town2')} 
        className={`w-full px-4 py-4 rounded-2xl text-left text-xs font-black uppercase tracking-widest transition-all flex items-center justify-between group mt-1 ${activeScene === 'town2' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-300 hover:bg-white/5'}`}
      >
        <span>Town 2</span>
        {activeScene === 'town2' ? (
          <div className="w-1.5 h-1.5 rounded-full bg-white animate-pulse" />
      ...
      <button 
        onClick={() => onTravel('darkest')} 
        className={`w-full px-4 py-4 rounded-2xl text-left text-xs font-black uppercase tracking-widest transition-all flex items-center justify-between group mt-1 ${activeScene === 'darkest' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-300 hover:bg-white/5'}`}
      >
        <span>Darkest Clone</span>
        {activeScene === 'darkest' ? (
          <div className="w-1.5 h-1.5 rounded-full bg-white animate-pulse" />
        ) : (
          <div className="w-1.5 h-1.5 rounded-full bg-white/10 group-hover:bg-white/30" />
