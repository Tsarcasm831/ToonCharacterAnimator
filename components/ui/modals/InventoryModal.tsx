...
const ITEM_TO_SLOT_MAP: Record<string, string> = {
    ...
    'Sword': 'weapon', 'Axe': 'weapon', 'Pickaxe': 'weapon', 'Shovel': 'weapon', 'Knife': 'weapon',
...
export const InventoryModal: React.FC<InventoryModalProps> = ({
    ...
    useEffect(() => {
        ...
        const frame = window.requestAnimationFrame(() => setShowPreview(true));
        return () => window.cancelAnimationFrame(frame);
    }, [isOpen]);

    const stats = useMemo(() => getStats(config, equipmentSlots), [config, equipmentSlots]);
    const hotbarItems = useMemo(() => inventory.slice(0, 10), [inventory]);
    const mainInventoryItems = useMemo(() => inventory.slice(10, 60), [inventory]);
    ...
    const handleDragStart = (e: React.DragEvent, index: number, item: InventoryItem, isHotbar: boolean) => {
        const actualIndex = isHotbar ? index : index + 10;
    ...
    const handleDrop = (e: React.DragEvent, targetDisplayIndex: number, isHotbar: boolean) => {
        e.preventDefault();
        const targetIndex = isHotbar ? targetDisplayIndex : targetDisplayIndex + 10;
    ...
    const handleMouseEnter = useCallback((e: React.MouseEvent, itemName: string) => {
        if (isEditMode) return;
        ...
        return (
            <div
                ...
                className={`
                    ...
                    ${isHotbar ? 'h-12 w-12' : 'h-[clamp(2.5rem,5vw,3.5rem)] w-[clamp(2.5rem,5vw,3.5rem)]'}
                    ${item
            ...
            >
                ...
                {isHotbar && <span className="absolute bottom-0.5 right-1 text-[8px] text-stone-500 font-mono z-10">{index === 9 ? '0' : index + 1}</span>}
            </div>
        );
    };
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm p-3 md:p-4">
            ...
            {/* Main Window Frame */}
            <div className="h-[min(860px,calc(100vh-24px))] w-[min(1040px,calc(100vw-32px))] bg-[#0c0a09] border-2 border-[#4a3b2a] shadow-[0_0_50px_rgba(0,0,0,0.8)] relative flex flex-col font-serif">
                ...
                {/* --- Main Content Area --- */}
                <div className="flex-1 min-h-0 flex flex-col p-1 overflow-hidden">
                    ...
                    >
                        ...
                        <div className="absolute inset-0 flex items-center justify-center opacity-75 pointer-events-none grayscale sepia-[0.2]">
                             {showPreview ? (
                                <div className="scale-110 transform translate-y-2">
                                    <PlayerPreview config={config} />
                                </div>
                             ) : (
                                <div className="h-48 w-28 border border-stone-800/60 bg-black/30" />
                             )}
                        </div>
                        ...
                        {EQUIPMENT_SLOTS.map((slot) => {
                            ...
                            return (
                                ...
                                >
                                    {isEquipped ? (
                                        equippedImage ? (
                                            <img src={equippedImage} alt={equippedId} loading="lazy" decoding="async" className={`h-full w-full object-contain p-1 ${isEditMode ? 'opacity-50' : ''}`} />
                                        ) : (
                    ...
                    <div className="bg-[#0f0e0d] p-3 flex-1 min-h-0 border border-[#292524] m-1 mt-0 relative overflow-y-auto custom-scrollbar shadow-inner">
                        {activeTab === 'INVENTORY' ? (
                            <div className="grid grid-cols-[repeat(10,clamp(2.5rem,5vw,3.5rem))] auto-rows-[clamp(2.5rem,5vw,3.5rem)] gap-1 justify-center content-start">
                                {mainInventoryItems.map((item, i) => renderGridSlot(item, i, false))}
                                {Array.from({ length: 50 - mainInventoryItems.length }).map((_, i) => (
                                    <div key={`empty-${i}`} className="h-[clamp(2.5rem,5vw,3.5rem)] w-[clamp(2.5rem,5vw,3.5rem)] bg-black/20 border border-stone-800/30 border-dashed rounded-sm" />
                                ))}
                            </div>
                        ) : (
