...
import { BODY_PRESETS, ITEM_DATA, OUTFIT_PRESETS } from '../../../data/constants';
...
export const Game: React.FC = () => {
    ...
    const { 
        ...
        isInventoryOpen, isItemCatalogOpen, isTradeOpen, isShopkeeperChatOpen, isForgeOpen, isKeybindsOpen, isQuestLogOpen, isSpawnModalOpen, isEnemiesModalOpen, isCharacterStatsOpen, isLandMapOpen, isAreaMapOpen, isTravelOpen,
    ...
    const tutorialThreePigSpawnedRef = React.useRef(false);
    ...
    React.useEffect(() => {
        if (tutorialStep !== 3) {
            tutorialThreePigSpawnedRef.current = false;
        }
    }, [tutorialStep]);
    ...
    React.useEffect(() => {
        if (tutorialStep !== 3 || activeScene !== 'starter' || tutorialThreePigSpawnedRef.current) return;
        if (!gameInstance.current?.player?.mesh || !gameInstance.current?.entityManager) return;

        tutorialThreePigSpawnedRef.current = true;
        ...
        console.log('[Game.tsx] Spawning tutorial quest 3 pigs near player at position:', playerPos);
    ...
    }, [tutorialStep, activeScene, gameInstance]);
    ...
    const decrementInventorySlot = React.useCallback((slotIndex: number, itemName: string) => {
        setInventory(prev => {
            const current = prev[slotIndex];
            if (!current || current.name !== itemName) return prev;
    ...
    const handleHotbarSlotSelect = React.useCallback((slotIndex: number) => {
        const item = inventory[slotIndex];

        if (slotIndex === 8 && item?.name === 'Potion of Healing') {
            const amount = ITEM_DATA[item.name]?.stats?.healthBonus ?? 0;
            if (amount > 0 && config.stats.health < config.stats.maxHealth) {
                setConfig(prev => ({
                    ...prev,
                    stats: {
                        ...prev.stats,
                        health: Math.min(prev.stats.maxHealth, prev.stats.health + amount),
                    },
                }));
                decrementInventorySlot(slotIndex, item.name);
                setNotification('Drank Potion of Healing');
            } else {
                setNotification('Health is already full');
            }
            return;
        }

        if (slotIndex === 9 && item?.name === 'Mana Potion') {
            const amount = ITEM_DATA[item.name]?.stats?.manaBonus ?? 0;
            if (amount > 0 && config.stats.mana < config.stats.maxMana) {
                setConfig(prev => ({
                    ...prev,
                    stats: {
                        ...prev.stats,
                        mana: Math.min(prev.stats.maxMana, prev.stats.mana + amount),
                    },
                }));
                decrementInventorySlot(slotIndex, item.name);
                setNotification('Drank Mana Potion');
            } else {
                setNotification('Mana is already full');
            }
            return;
        }

        setSelectedSlot(slotIndex);
    }, [config.stats.health, config.stats.mana, config.stats.maxHealth, config.stats.maxMana, decrementInventorySlot, inventory, setConfig, setNotification, setSelectedSlot]);
    ...
    React.useEffect(() => {
        ...
        if (selectedItem) {
            ...
            const isWeapon = [
                'Sword', 'Axe', 'Pickaxe', 'Shovel', 'Knife', 'Halberd',
    ...
    const onGameReady = (game: any) => {
        ...
        game.inputManager.onToggleItemCatalog = uiState.toggleItemCatalog;
    ...
    if (isSupabaseEnabled && isAuthLoading) {
        return (
            <div className="w-full h-full min-h-0 flex items-center justify-center bg-[#080c14]">
    ...
    return (
        <div className="relative w-full h-full min-h-0 overflow-hidden flex flex-col items-center justify-start">
            <div ref={viewportRef} className="w-full flex-1 min-h-0 bg-black border-x border-t border-white/10 shadow-2xl overflow-hidden relative group">
                            ...
                            {showGlobalHUD && (
                                <>
                                    ...
                                    <GameHUD 
                                        ...
                                        onSelectSlot={handleHotbarSlotSelect}
