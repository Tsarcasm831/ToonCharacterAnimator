...
void main() {
    ...
    vec3 finalColor = mix(vColor, uCoreColor, plasma) * max(0.001, uIntensity);
    gl_FragColor = vec4(finalColor, (0.4 + plasma) * endFade * max(0.001, uIntensity));
}
`;
...
void main() {
    ...
    vec3 finalColor = mix(vColor, uCoreColor, fresnel) * max(0.001, uIntensity);
    gl_FragColor = vec4(finalColor, (0.7 + fresnel * 0.3) * max(0.001, uIntensity));
}
`;
...
export class ChakraNetwork {
    ...
    private coreColor = new THREE.Color(0x44aaff);
    ...
    setAppearance(color: THREE.ColorRepresentation, intensity: number) {
        this.coreColor.set(color);
        this.intensity = THREE.MathUtils.clamp(intensity, 0, 5);

        if (this.beamMesh) {
            const beamMat = this.beamMesh.material as THREE.ShaderMaterial;
            beamMat.uniforms.uCoreColor.value.copy(this.coreColor);
            beamMat.uniforms.uIntensity.value = this.intensity;
        }
        if (this.orbMesh) {
            const orbMat = this.orbMesh.material as THREE.ShaderMaterial;
            orbMat.uniforms.uCoreColor.value.copy(this.coreColor);
            orbMat.uniforms.uIntensity.value = this.intensity;
        }
    }
