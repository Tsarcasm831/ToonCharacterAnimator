...
export class RobeBuilder {
    static build(parts: any, config: PlayerConfig) {
        ...
        const layerScale = 1.12; // Keep the robe fitted over the body instead of ballooning outward

        // --- UPPER BODY (Torso) ---
        const torsoRadiusTop = 0.305;
        const torsoRadiusBottom = 0.315;
        const robeLen = 0.61;
        const torsoDepthScale = 0.74;
        ...
        for(let i=0; i<pos.count; i++) {
            ...
            // Create a front neckline without dropping the whole top shell too far.
            if (v.z > 0 && v.y > robeLen / 2 - 0.16) {
                const t = (v.y - (robeLen / 2 - 0.16)) / 0.16;
                const centerBias = Math.max(0, 1.0 - Math.abs(v.x) / 0.14);
                v.y -= centerBias * t * 0.08;
                v.z -= centerBias * t * 0.03;
            }

            // Slight shoulder broadening to meet the sleeve root cleanly.
            if (v.y > robeLen / 2 - 0.16) {
                const t = (v.y - (robeLen / 2 - 0.16)) / 0.16;
                v.x *= 1.0 + t * 0.035;
                v.z *= 1.0 + t * 0.015;
            }

            // Taper the top back in so the collar sits around the neck instead of flaring.
            if (v.y > robeLen/2 - 0.08) {
                const t = (v.y - (robeLen/2 - 0.08)) / 0.08;
                v.x *= (1.0 - t * 0.12);
                v.z *= (1.0 - t * 0.1);
            }

            // Give the lower torso a small overlap into the skirt, but keep it fitted.
            if (v.y < -robeLen / 2 + 0.18) {
                const t = (-robeLen / 2 + 0.18 - v.y) / 0.18;
                v.x *= 1.0 + t * 0.035;
                v.z *= 1.0 + t * 0.02;
            }
        ...
        // --- SHOULDER MANTLE ---
        // A dedicated mantle prevents the shirt from poking through the upper chest/shoulder area.
        const mantleHeight = 0.16;
        const mantleTopRad = torsoRadiusTop * layerScale * 1.03;
        const mantleBottomRad = mantleTopRad * 1.16;
        const mantleGeo = new THREE.CylinderGeometry(
            mantleTopRad,
            mantleBottomRad,
            mantleHeight,
            32,
        ...
        mantleGeo.scale(1, 1, torsoDepthScale * 1.02);

        const mantlePos = mantleGeo.attributes.position;
        const mv = new THREE.Vector3();
        for (let i = 0; i < mantlePos.count; i++) {
            mv.fromBufferAttribute(mantlePos, i);
            if (mv.y > 0) {
                const t = mv.y / (mantleHeight / 2);
                mv.x *= 0.96 - t * 0.04;
                mv.z *= 0.98 - t * 0.03;
            }
            if (mv.z > 0) {
                const frontBias = Math.max(0, 1.0 - Math.abs(mv.x) / (mantleBottomRad * 0.72));
                mv.y -= frontBias * 0.03;
                mv.z -= frontBias * 0.015;
            }
            mantlePos.setXYZ(i, mv.x, mv.y, mv.z);
        }
        mantleGeo.computeVertexNormals();

        const mantle = new THREE.Mesh(mantleGeo, robeMat);
        mantle.position.set(0, robeLen / 2 - 0.03, 0);
        robeTorso.add(mantle);
        createdMeshes.push(mantle);
        scaleUVs(mantle, mantleBottomRad, mantleHeight);

        // Top yoke cover to hide the shirt peeking through between the mantle and neck.
        const yokeGeo = new THREE.SphereGeometry(
            torsoRadiusTop * layerScale * 0.98,
            20,
            14,
            0,
            Math.PI * 2,
        ...
        yokeGeo.scale(1.26, 0.22, torsoDepthScale * 1.08);
        const yokePos = yokeGeo.attributes.position;
        const yv = new THREE.Vector3();
        for (let i = 0; i < yokePos.count; i++) {
            yv.fromBufferAttribute(yokePos, i);
            if (yv.z > 0) {
                const centerBias = Math.max(0, 1.0 - Math.abs(yv.x) / 0.22);
                yv.y -= centerBias * 0.035;
                yv.z -= centerBias * 0.01;
            }
            yokePos.setXYZ(i, yv.x, yv.y, yv.z);
        }
        yokeGeo.computeVertexNormals();

        const yoke = new THREE.Mesh(yokeGeo, robeMat);
        yoke.position.set(0, robeLen / 2 + 0.012, -0.005);
        robeTorso.add(yoke);
        createdMeshes.push(yoke);
        scaleUVs(yoke, torsoRadiusTop * layerScale, torsoRadiusTop * layerScale * 0.6);

        // Visible shoulder caps that sit over the sleeve roots.
        [-1, 1].forEach((side) => {
            const capGeo = new THREE.SphereGeometry(0.14, 16, 12, 0, Math.PI * 2, 0, Math.PI * 0.5);
            capGeo.scale(1.35, 0.55, 1.0);
            const shoulderCap = new THREE.Mesh(capGeo, robeMat);
            shoulderCap.position.set(side * 0.285, robeLen / 2 - 0.045, 0.01);
            shoulderCap.rotation.z = -side * 0.22;
            shoulderCap.rotation.y = side * 0.08;
            robeTorso.add(shoulderCap);
            createdMeshes.push(shoulderCap);
            scaleUVs(shoulderCap, 0.14, 0.14);
        });

        // Collar/Neck Trim - decorative ring around the neckline
        const collarRadius = torsoRadiusTop * layerScale * 0.3;
        const collarGeo = new THREE.TorusGeometry(collarRadius, 0.02, 8, 32);
        collarGeo.scale(1, 1, torsoDepthScale * 0.9);
        
        const collar = new THREE.Mesh(collarGeo, trimMat);
        collar.rotation.x = Math.PI / 2;
        collar.position.set(0, robeLen / 2 + 0.03, -0.01);
        robeTorso.add(collar);
        createdMeshes.push(collar);

        // --- SASH (BELT) ---
        // Thick cloth belt to hide the seam between upper/lower and cinch the waist
        const sashGeo = new THREE.TorusGeometry(torsoRadiusBottom * layerScale * 0.97, 0.065, 8, 36);
        sashGeo.scale(1, 1, torsoDepthScale);
        
        const sash = new THREE.Mesh(sashGeo, sashMat);
        sash.position.y = -robeLen/2 + 0.07;
        ...
        armPairs.forEach(({ arm }) => {
            ...
            sleeveGeo.translate(0, -sLen * 0.45 + 0.005, 0);
        ...
        if (parts.pelvis) {
            ...
            skirt.position.y = -skirtLen/2 + 0.06;
            ...
            const tabardWidth = 0.28;
            const tabardLen = skirtLen * 0.94;
            ...
            tabard.position.set(0, -tabardLen/2 + 0.06, skirtTopRad * torsoDepthScale + 0.015);
