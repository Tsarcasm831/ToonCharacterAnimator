    ...
    const equipmentSections: { title: string; items: { id: keyof PlayerConfig['equipment']; label: string }[] }[] = [
        {
            ...
            items: [
                ...
                { id: 'plagueDoctorMask', label: 'Plague Doctor' },
                { id: 'mask', label: 'Mask' }
            ]
        },
        {
                ...
                <div className="space-y-4 pt-4 border-t border-white/5 bg-black/20 p-4 rounded-xl">
                    <div className="grid grid-cols-1 gap-4">
                        ...
                        {config.equipment.plagueDoctorMask && (
                            <>
                                <ColorPicker label="Plague Leather" value={config.plagueMaskColor} onChange={(v) => handleConfigChange('plagueMaskColor', v)} />
                                <ColorPicker label="Plague Trim" value={config.plagueMaskTrimColor} onChange={(v) => handleConfigChange('plagueMaskTrimColor', v)} />
                                <ColorPicker label="Plague Lens" value={config.plagueMaskLensColor} onChange={(v) => handleConfigChange('plagueMaskLensColor', v)} />
                            </>
                        )}
