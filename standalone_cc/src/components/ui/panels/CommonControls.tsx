...
export const ColorPicker: React.FC<{ label: string, value: string, onChange: (v: string) => void }> = ({ label, value, onChange }) => {
    ...
    const inputName = label.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '') || 'color_value';

    return (
        <div className="flex items-center justify-between group">
            <label htmlFor={inputId} className="text-[10px] font-black text-slate-400 uppercase tracking-widest group-hover:text-slate-200 transition-colors">{label}</label>
...
export const Slider: React.FC<{ label: string, value: number, min: number, max: number, step: number, onChange: (val: number) => void }> = ({ label, value, min, max, step, onChange }) => {
  const inputId = React.useId();
  const inputName = label.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '') || 'slider_value';
  ...
  return (
    <div className="flex flex-col gap-2 group">
      <div className="flex justify-between items-end">
          <label htmlFor={inputId} className="text-[10px] font-black text-slate-500 uppercase tracking-widest group-hover:text-slate-300 transition-colors">{label}</label>
