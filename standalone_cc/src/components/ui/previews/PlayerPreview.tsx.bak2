...
const createSpiderPreviewModel = (colorHex: string): CreaturePreviewModel => {
    const baseColor = new THREE.Color(parseColor(colorHex, 0x173f67));
    const darkColor = baseColor.clone().multiplyScalar(0.5);
    const furColor = baseColor.clone().lerp(new THREE.Color(0x7ea7cf), 0.25);

    const group = new THREE.Group();
    const shellMat = new THREE.MeshStandardMaterial({ color: baseColor, flatShading: true, roughness: 0.9, metalness: 0.04 });
    const shellDarkMat = new THREE.MeshStandardMaterial({ color: darkColor, flatShading: true, roughness: 0.92, metalness: 0.02 });
    const furMat = new THREE.MeshStandardMaterial({ color: furColor, flatShading: true, roughness: 0.98, metalness: 0.0 });
    const legMat = new THREE.MeshStandardMaterial({ color: 0x0d131d, flatShading: true, roughness: 0.85, metalness: 0.06 });
    const eyeMat = new THREE.MeshBasicMaterial({ color: 0x8dff2a });
    const smallEyeMat = new THREE.MeshBasicMaterial({ color: 0xeaf5ff });

    const abdomen = new THREE.Mesh(new THREE.SphereGeometry(0.29, 16, 12), shellMat);
    abdomen.scale.set(1.18, 0.9, 1.28);
    abdomen.position.set(0, 0.44, -0.08);
    abdomen.castShadow = true;
    group.add(abdomen);

    const cephalothorax = new THREE.Mesh(new THREE.SphereGeometry(0.24, 16, 12), shellDarkMat);
    cephalothorax.scale.set(1.1, 0.82, 1.2);
    cephalothorax.position.set(0, 0.42, 0.21);
    cephalothorax.castShadow = true;
    group.add(cephalothorax);

    const faceMuzzle = new THREE.Mesh(new THREE.SphereGeometry(0.14, 12, 10), shellDarkMat);
    faceMuzzle.scale.set(1.35, 0.8, 0.95);
    faceMuzzle.position.set(0, 0.39, 0.41);
    group.add(faceMuzzle);

    const eyeL = new THREE.Mesh(new THREE.SphereGeometry(0.062, 12, 10), eyeMat);
    const eyeR = new THREE.Mesh(new THREE.SphereGeometry(0.062, 12, 10), eyeMat);
    eyeL.scale.set(0.9, 1.0, 0.56);
    eyeR.scale.set(0.9, 1.0, 0.56);
    eyeL.position.set(-0.07, 0.405, 0.49);
    eyeR.position.set(0.07, 0.405, 0.49);
    group.add(eyeL, eyeR);

    const upperEyePositions = [
        [-0.11, 0.5, 0.38],
        [-0.038, 0.53, 0.37],
        [0.038, 0.53, 0.37],
        [0.11, 0.5, 0.38],
    ] as const;
    upperEyePositions.forEach(([x, y, z]) => {
        const e = new THREE.Mesh(new THREE.SphereGeometry(0.02, 8, 8), smallEyeMat);
        e.position.set(x, y, z);
        group.add(e);
    });

    const fangGeo = new THREE.ConeGeometry(0.011, 0.07, 5);
    const fangL = new THREE.Mesh(fangGeo, legMat);
    const fangR = new THREE.Mesh(fangGeo, legMat);
    fangL.rotation.x = Math.PI * 0.82;
    fangR.rotation.x = Math.PI * 0.82;
    fangL.position.set(-0.025, 0.335, 0.38);
    fangR.position.set(0.025, 0.335, 0.38);
    group.add(fangL, fangR);

    const addFurRing = (center: THREE.Vector3, radius: number, count: number, lift = 0.04) => {
        for (let i = 0; i < count; i++) {
            const angle = (i / count) * Math.PI * 2;
            const tuft = new THREE.Mesh(new THREE.ConeGeometry(0.005, 0.04 + (i % 3) * 0.01, 4), furMat);
            const x = center.x + Math.cos(angle) * radius;
            const y = center.y + lift + Math.sin(angle * 2) * 0.01;
            const z = center.z + Math.sin(angle) * radius * 0.9;
            tuft.position.set(x, y, z);
            tuft.lookAt(center.x + Math.cos(angle) * (radius + 0.2), y + 0.03, center.z + Math.sin(angle) * (radius + 0.2));
            group.add(tuft);
        }
    };
    addFurRing(new THREE.Vector3(0, 0.44, 0.2), 0.2, 16, 0.03);
    addFurRing(new THREE.Vector3(0, 0.45, -0.08), 0.27, 22, 0.02);
    ...
    const femurGeo = new THREE.BoxGeometry(0.26, 0.038, 0.038);
    const tibiaGeo = new THREE.BoxGeometry(0.28, 0.032, 0.032);
    const tarsusGeo = new THREE.BoxGeometry(0.24, 0.028, 0.028);

    for (let i = 0; i < 4; i++) {
        const z = 0.31 - i * 0.21;
        const baseSweep = i < 2 ? 0.42 - i * 0.07 : -0.24 - (i - 2) * 0.07;
        const baseSpread = 0.5 + i * 0.08;
        ([-1, 1] as const).forEach((side) => {
            const root = new THREE.Group();
            root.position.set(0.13 * side, 0.4, z);

            const femur = new THREE.Mesh(femurGeo.clone().translate(0.13 * side, 0, 0), legMat);
            const tibia = new THREE.Mesh(tibiaGeo.clone().translate(0.14 * side, 0, 0), legMat);
            const tarsus = new THREE.Mesh(tarsusGeo.clone().translate(0.12 * side, 0, 0), legMat);

            femur.castShadow = true;
            tibia.castShadow = true;
            tarsus.castShadow = true;

            femur.rotation.z = side * baseSpread;
            femur.rotation.y = side * baseSweep;
            tibia.position.set(0.26 * side, 0, 0);
            tibia.rotation.z = side * -1.0;
            tarsus.position.set(0.28 * side, 0, 0);
            tarsus.rotation.z = side * -0.34;
            ...
            legRigs.push({ side, index: i, root, femur, tibia, tarsus, baseSpread, baseSweep });
        });
    }
    ...
    return {
        ...
        animate: (time) => {
            const t = time * 2.4;
            const bodyBob = -Math.abs(Math.sin(t)) * 0.014;
            abdomen.position.y = 0.44 + bodyBob;
            cephalothorax.position.y = 0.42 + bodyBob * 0.75;
            faceMuzzle.position.y = 0.39 + bodyBob * 0.75;

            legRigs.forEach((leg) => {
                const tripodPhase = ((leg.index + (leg.side === 1 ? 1 : 0)) % 2) * Math.PI;
                const drive = Math.sin(t + tripodPhase);
                const lift = Math.max(0, drive);

                leg.femur.rotation.y = leg.side * (leg.baseSweep + drive * 0.15);
                leg.femur.rotation.z = leg.side * (leg.baseSpread + lift * 0.14);
                leg.tibia.rotation.z = leg.side * (-1.0 - lift * 0.28);
                leg.tarsus.rotation.z = leg.side * (-0.34 + lift * 0.1);
            });

            eyeL.scale.z = 0.56 + Math.sin(t * 1.15) * 0.015;
            eyeR.scale.z = 0.56 + Math.sin(t * 1.15 + 0.25) * 0.015;
        },
        applyColor: (nextColorHex) => {
            const nextBase = new THREE.Color(parseColor(nextColorHex, 0x173f67));
            shellMat.color.copy(nextBase);
            shellDarkMat.color.copy(nextBase).multiplyScalar(0.5);
            furMat.color.copy(nextBase).lerp(new THREE.Color(0x7ea7cf), 0.25);
        },
    };
};
